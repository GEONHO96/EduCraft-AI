...
| 기능 | 설명 |
|:-----|:-----|
| 학년별 AI 퀴즈 | 초등~고등 12학년 x 국/영/수/코딩 맞춤 퀴즈 (280+ 문제 내장) |
...
| 커뮤니티 피드 | 인스타그램 스타일 피드, 스토리 바, 더블탭 좋아요, 댓글, 팔로우 |
...
| 학년별 퀴즈 | 오프라인 전용 (280+ 문제) | 오프라인 전용 (280+ 문제) |
...
| 오프라인 엔진 | 14과목 템플릿 + 280+ 문제 은행 + 키워드 챗봇 |
...
flowchart LR
    ...
    subgraph System["🏫 EduCraft AI"]
        ...
        subgraph TOnly["교강사 전용"]
            ...
            T3["AI 수업 자료 생성"]
            T4["AI 퀴즈 출제"]
            T5["교강사 대시보드<br/>수강생 성취도 · 활동 통계 · 상세 패널"]
            T6["배치 작업 실행"]
        end
        ...
        subgraph SOnly["학생 전용"]
            ...
            S2["커리큘럼 · 학습자료 열람"]
            S3["퀴즈 풀기 · 자동 채점"]
            S4["학년별 AI 퀴즈<br/>초등~고등 12학년 × 국/영/수/코딩"]
...
| 페이지 | 경로 | 설명 |
|:-------|:-----|:-----|
| 로그인 | `/login` | 몽환적 그린 브랜딩 패널 + Validation 폼, 소셜 로그인 (Google/Kakao/Naver) |
| 회원가입 | `/register` | 그린 테마 스플릿 레이아웃, 역할 선택, 학년 선택, 실시간 이메일 중복·비밀번호 강도 검사 |
| 소셜 콜백 | `/auth/:provider/callback` | OAuth 후 역할 선택, 자동 로그인 |
| 계정 찾기 | `/find-account` | 그린 테마 이메일 찾기 / 비밀번호 재설정 (3단계) |
| 대시보드 | `/` | 교강사: 통계(클릭 시 상세)+빠른시작+활동 / 학생: 수강+성적 |
...
| 학년별 퀴즈 | `/grade-quiz` | 학년/과목(국영수코딩) 선택 → 즉시 풀기 |
| 강의 추천 | `/recommend` | 학년별 유튜브 교육 영상 |
| 요금제 | `/pricing` | 플랜 비교, 결제 (3단계 모달) |
| 커뮤니티 | `/sns/feed` | 인스타그램 스타일 피드, 스토리 바, 더블탭 좋아요, 댓글 |
...
| 항목 | 설명 |
|:-----|:-----|
| 반응형 레이아웃 | 모바일 햄버거 메뉴, 데스크톱 네비게이션 바 (축소 시 자동 스크롤) |
| 로딩 스켈레톤 | 데이터 로딩 시 콘텐츠 형태 스켈레톤 UI |
| 에러 바운더리 | 컴포넌트 오류 시 앱 크래시 방지, 재시도 버튼 |
| 토스트 알림 | react-hot-toast 기반 즉각적 피드백 |
| 인증 페이지 그린 테마 | 로그인/회원가입/계정찾기 통일된 몽환적 초록 브랜딩 패널 (50:50 스플릿) |
| 글래스모피즘 디자인 | 인증 브랜딩 패널에 반투명 유리 질감 카드, 빛 오브, 떠다니는 입자 애니메이션 |
| 실시간 폼 유효성 검사 | 이메일/비밀번호 onBlur 검증, 에러 아이콘+메시지, 비밀번호 강도 pill |
| 인스타그램 스타일 피드 | 스토리 바, 더블탭 좋아요, 이미지 우선 카드, 댓글 프리뷰, "더 보기" 텍스트 확장 |
| 과목별 SVG 아이콘 | 수학(계산기), 영어(지구본), 과학(플라스크) 등 과목별 커스텀 SVG 아이콘 |
| 대시보드 상세 패널 | 통계 카드 클릭 시 상세 정보 패널 펼침 (강의/수강생/AI/활동별 개별 표시) |
| 클릭 가능 카드 | 강의 카드, 커리큘럼 카드, 자료 핵심포인트 등 전체 영역 클릭 지원 |
...
| 효과 측정 | 불가능 | AI 생성 횟수 · 주간 활동 · 자료/퀴즈 현황 정량 측정 |
