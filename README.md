...
- **닉네임 변경과 프로필 이미지 업로드**로 개인화된 학습 환경을 제공합니다
- **구독 요금제 시스템**으로 Community / Pro / Max 플랜을 제공합니다
- **API 키 없이도** 커리큘럼/자료/퀴즈 생성, 학년별 퀴즈, 강의 추천, 챗봇이 **오프라인으로 동작**합니다
...
| 기능 | 설명 |
|------|------|
| **AI 커리큘럼 설계** | 과목/주제/주차 수/대상 수준 입력 → 주차별 커리큘럼 자동 생성 (오프라인 14과목 템플릿 폴백) |
| **AI 수업 자료 생성** | 커리큘럼 기반 강의자료/실습자료 자동 생성 (난이도 조절, 오프라인 폴백 지원) |
| **AI 퀴즈 출제** | 객관식/주관식 문제 자동 생성, 해설 포함 (오프라인 폴백 지원) |
...
| **프로필 수정** | 닉네임 변경, 프로필 이미지 업로드/삭제, 미리보기 |
...
AI API 키가 없거나 API가 응답하지 않을 때, **내장 템플릿 기반으로 자동 전환**되어 모든 AI 생성 기능이 동작합니다.

| 기능 | 온라인 (API 키 있음) | 오프라인 (API 키 없음) |
|------|---------------------|----------------------|
| **커리큘럼 생성** | Claude API가 맞춤 설계 | 14과목 내장 템플릿에서 키워드 매칭 후 생성 |
| **수업 자료 생성** | Claude API가 강의/실습 자료 작성 | 3개 섹션 구조화된 템플릿 자료 생성 |
| **퀴즈 출제** | Claude API가 문제 출제 | 객관식+주관식 혼합 템플릿 문제 생성 |
| **에듀봇 챗봇** | Claude API 실시간 답변 | 14개 카테고리 키워드 매칭 응답 |
| **학년별 퀴즈** | 오프라인 전용 (210+ 내장 문제) | 오프라인 전용 (210+ 내장 문제) |
| **강의 추천** | 오프라인 전용 (유튜브 임베드) | 오프라인 전용 (유튜브 임베드) |

### 내장 커리큘럼 템플릿 (14과목)

`Java OOP` · `Python` · `웹 개발` · `React 프론트엔드` · `Spring 백엔드` · `데이터베이스` · `자료구조` · `알고리즘` · `C 프로그래밍` · `모바일 앱` · `네트워크` · `운영체제` · `머신러닝` · `수학`

> 과목/주제 키워드를 분석하여 최적 템플릿을 자동 선택합니다. 매칭되지 않는 주제는 범용 템플릿으로 생성됩니다.
...
| 오프라인 커리큘럼 엔진 | 14과목 템플릿 기반 커리큘럼/자료/퀴즈 오프라인 생성 (API 키 없이 동작) |
...
│       └── infra/
│           └── ai/             # Claude API 클라이언트, 오프라인 커리큘럼 템플릿 (14과목)
│
└── frontend/
    ├── package.json
    ├── Dockerfile
    └── src/
        ...
        └── pages/
            ├── auth/           # 로그인, 회원가입(학년 선택), 소셜로그인, 계정찾기, 프로필 수정
...
> 총 **49개** 엔드포인트 | 공통 응답 형식: `{ "success": true, "data": {...} }` 또는 `{ "success": false, "error": {"code": "...", "message": "..."} }`  
...
{
  "nickname": "에듀크래프터",
  "profileImage": "http://localhost:8080/uploads/profile.jpg"
}
```
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| nickname | String | X | 닉네임 (최대 20자, null이면 기본 이름으로 복원) |
...
| **프로필 수정** | `/profile/edit` | 닉네임 변경(20자 제한), 프로필 이미지 업로드/삭제, 이메일·역할 읽기 전용 표시 |
...
| **프로필 이미지 업로드** | 호버 오버레이 카메라 아이콘, 이미지 미리보기, 업로드 스피너, 삭제 지원 |
...
| 프로필 커스텀 | 제한적 | **닉네임 변경 + 프로필 이미지 업로드 (소셜 로그인 이름 덮어쓰기)** |
| 오프라인 지원 | 불가능 | **API 키 없이도 커리큘럼/자료/퀴즈 생성, 학년별 퀴즈, 강의 추천, 챗봇 모두 동작** |
